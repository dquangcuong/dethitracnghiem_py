...
def show_exam_manager(root):
    ...
    def add():
        ...
        # Chọn môn
        sw = tk.Toplevel(window); sw.title("Chọn môn học"); sw.geometry("300x120"); sw.configure(bg="#cce6ff"); sw.grab_set()
        ...
        try:
            ...
            messagebox.showerror("Lỗi", "Số không hợp lệ.", parent=window); return
    ...
    def view():
        ...
        if not ex:
            messagebox.showerror("Lỗi", "Không tìm thấy.", parent=window); return

        qmap = {q["id"]:q for q in load_questions()}
        vw = tk.Toplevel(window); vw.title(f"Chi tiết: {ex['name']}"); vw.geometry("700x500"); vw.configure(bg="#cce6ff")
        cv = tk.Canvas(vw, bg="#cce6ff"); sb = tk.Scrollbar(vw, command=cv.yview); fr = tk.Frame(cv, bg="#cce6ff")
        ...
        def save_q(qid, txt, ans, opts):
            qq = qmap[qid]
            qq["question"]=txt; qq["answer"]=ans; qq["options"]=opts
        ...
        for i,qid in enumerate(ex["questions"],1):
            q = qmap.get(qid)
            if not q: continue
            f = tk.Frame(fr, bg="white", bd=1, relief="solid", padx=5, pady=5); f.pack(fill="x", pady=5)
            tk.Label(f, text=f"Câu {i}:", font=("Segoe UI",12,"bold"), bg="white").pack(anchor="w")
            txt = tk.Text(f, height=3, wrap="word", font=("Segoe UI",11)); txt.insert("1.0",q["question"]); txt.pack(fill="x")
            tk.Label(f, text="Đáp án đúng:", font=("Segoe UI",10), bg="white").pack(anchor="w")
            ae = tk.Entry(f, font=("Segoe UI",11)); ae.insert(0,q["answer"]); ae.pack(anchor="w")
            ...
            for idx,opt in enumerate(q.get("options",[])):
                ...
                e=tk.Entry(f,font=("Segoe UI",11), width=100); e.insert(0,opt); e.pack(anchor="w",pady=2); opts.append(e)
            btn = ttk.Button(f, text="💾 Lưu", style="Rounded.TButton",
                             command=lambda qid=qid, t=txt, a=ae, os=opts: save_q(qid, t.get("1.0","end").strip(), a.get().strip(), [o.get().strip() for o in os]))
            btn.pack(pady=5)

    # Nút chức năng
    for txt, cmd, ico in [
        (f"{icon_add} Tạo đề thi", add, icon_add),
        (f"{icon_del} Xoá đề thi", delete, icon_del),
        (f"{icon_view} Xem đề thi", view, icon_view)
    ]:
        ttk.Button(window, text=txt, style="Rounded.TButton", command=cmd).pack(pady=5)

    def on_close():
        window.destroy()
        root.deiconify()  # hiện lại form main

    ttk.Button(window, text=f"{icon_close} Đóng", style="Rounded.TButton", command=on_close).pack(pady=10)

    load()
    window.protocol("WM_DELETE_WINDOW", on_close)  # bắt sự kiện đóng cửa sổ bằng [X]
    window.mainloop()
